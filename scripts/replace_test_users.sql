-- Delete existing test users and insert complete data
-- This removes users with the test email patterns and replaces them with complete records

-- Delete existing test users (those with test email patterns)
DELETE FROM users WHERE 
    Email LIKE '%pwd-reset-sessions-%' OR
    Email LIKE '%timing-test-user%' OR
    Email LIKE '%ratelimit-test%' OR
    Email LIKE '%fk-test%' OR
    Email LIKE '%newuser_%' OR
    Email LIKE '%password-reset-sessions-test%';

-- Insert complete test user records
INSERT INTO users (
    "Id", 
    "Email", 
    "PasswordHash", 
